...
This is MangaReader, a React + Vite manga reading management system for publisher.
...
- **All variable, function, and parameter names must be English** (no Vietnamese or accented characters).
...
- If file length is larger than 300 lines, split it.

#### Layout Components

- **Always break down layouts into smaller components** when possible (e.g., Header, Sidebar, Footer, Content, PageHeader, etc.).
- Each layout component should have a single, clear responsibility.
- Layout components should be reusable and composable.
- Common layout components (used across multiple features) should be placed in `./src/components/layout`.
- Feature-specific layout components should be placed inside their feature folder (e.g., `features/dashboard/components/layout`).
- Prefer composition over large monolithic layout components.
...
- Extract repeated layout patterns into reusable components.
- Use layout components to maintain consistent spacing and structure.
- Keep layout components focused on structure, not business logic.
- Pass content via `children` prop or named slots when appropriate.
...
- Reading `STYLING_GUIDE.md` for styling guides
